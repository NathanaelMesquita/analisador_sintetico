...
logo_esquerda = os.path.join(caminho_base, "logo_esquerda.jpg")
st.write(logo_esquerda)  # Adiciona esta linha para imprimir o caminho do arquivo

logo_direita = os.path.join(caminho_base, "logo_direita.jpg")

# Adiciona logotipo redimensionado e posiciona acima do título
col1, _, col2 = st.columns(
    [1, 5, 1])  # Configuração para exibir dois logotipos nas extremidades superior esquerda e direita
...
if st.button("Gerar Análise"):
    if principais_envolvidos is not None and informacoes_adicionais is not None:
        ...
        # Verificar e criar o arquivo de saída se não existir
        if not os.path.exists(output_file):
            wb = Workbook()
            wb.save(output_file)
            st.write(f"Arquivo '{output_file}' criado com sucesso.")
        ...
        for envolvido_cpf_cnpj in envolvidos:
            ...
            with pd.ExcelWriter(output_file, engine="openpyxl", mode="a") as writer:
                ...
                # Filtrar dados de BENEFICIARIOS
